...
class TaskWarriorWorker(object):
    ...
    def initialize(self) -> None:
        tasks_pending = [
            ...
            if t.get("project", False) == self.filter_project
        ]
        tasks_completed = [
            t
        ...
        print(
            f"Found {len(tasks_completed)} completed tasks for project "
        ...
        print(
            f"Found {len(tasks_active)} active tasks for project "
            f"{self.filter_project} in Taskwarrior DB"
        )
        ...
        print(
            f"Found {len(tasks_pending)} pending and not active tasks for "
        ...
        if len(self.tasks_pending) != pending_count:
            print(
                "Seems you have duplicate descriptions in your pending "
                "taskwarrior tasks"
            )
        self.tasks_completed = {t["description"]: t for t in tasks_completed}
        if len(self.tasks_completed) != completed_count:
            print(
                "Seems you have duplicate descriptions in your completed "
                "taskwarrior tasks"
            )
        ...
        if len(self.tasks_active) != active_count:
            print(
                "Seems you have duplicate descriptions in your active "
                "taskwarrior tasks"
            )
