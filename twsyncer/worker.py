...
class Worker(object):
    ...
    def find_missing_tw_tasks(
        self, issues: List[Issue], tasks: Dict[str, dict]
    ) -> List[Issue]:
        ...
        for issue in issues:
            if issue.title in tasks:
                pairs_to_check.append(
                    {"issue": issue, "task": tasks[issue.title]}
                )
                print(
                    f"Found existing both in tw and in Github task: "
                    f"{issue.title}"
                )
        # TODO(sbog): convert it to generator
    ...
    def resolve(self) -> None:
        ...
        print(
            f"Found {len(issues_to_create_tw)} issues to create in "
        ...
        print(
            f"Found {len(pairs_to_check)} items with the same description "
    ...
    def github_create_issues(self, tasks: List[dict]) -> None:
        for task in tasks:
            ...
            print(
                f"  {self.gh.dry_run_prefix}Create new issue from task "
    ...
    def github_create_card_from_issue(
        self, card_column_name: str, issue: Issue
    ) -> ProjectCard:
    ...
    def get_columns_data(self) -> List[dict]:
        ...
        for column in project_columns.values():
            ...
            print(
                f"    Processing {cards.totalCount} cards in column "
                f"{column.name}"
            )
            for index, card in enumerate(cards):
                ...
                if type(card_content) != Issue:
                    print(
                        f"    Looks that card '{card}' content is not an "
    ...
    def github_modify_issues(self, datalist: List[dict]) -> None:
        ...
        for index, hsh in enumerate(datalist):
            ...
            if (
                issue_data["column_name"]
                not in self.gh.tw_to_github_states[task_status]
            ):
                ...
                print(
                    f'  {self.gh.dry_run_prefix}Move issue "{issue}" from '
